...
namespace ROR2_Pusky
{
    public static class Render
    {
        ...
        public static void DrawColoredDot(Vector2 position, Color color)
        {
            Color = color;
            DrawDot(position);    
        }
        ...
        {
            //var angle = Vector2.SignedAngle(from, to);
            //GUIUtility.RotateAroundPivot(position);
            //DrawBox(from, Vector2.right * (from - to).magnitude, false);
            //GUIUtility.RotateAroundPivot(-angle, from);
            GUI.DrawTexture(new Rect(position, new Vector2(1, 1)), Texture2D.whiteTexture, ScaleMode.StretchToFill);
        }
